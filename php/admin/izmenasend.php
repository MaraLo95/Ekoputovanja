...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	foreach($add as $value)
	{
		...
		// Kreiranje upita:
		//$q = "UPDATE destination SET country='$country',city='$city',price='$price',transport='$transport',transportway='$transportway',apartmans='$smestaj',apartmansway='$smestajmodel',nameapartamns='$smestajime',food='$hrana',foodway='$hranaway',description='$description',date=NOW(),datestart='$startdate',dateend='$enddate',add='$dodaj',pocetna='$pocetna',slideri='$slideri' WHERE id=$idizmena";
        $q2 = "DELETE FROM destination WHERE id = $idizmena";
		$q = "INSERT INTO `destination` (`id`,`country`,`city`,`price`,`transport`,`transportway`,`apartmans`,`apartmansway`,`nameapartamns`,`food`,`foodway`,`description`,`date`,`datestart`,`dateend`,`days`,`add`,`pocetna`,`slideri`) VALUES ('$idizmena','$country', '$city', '$price','$transport','$transportway' ,'$smestaj','$smestajmodel','$smestajime','$hrana','$hranaway','$description', NOW() ,'$startdate','$enddate','$brdana','$dodaj','$pocetna','$slideri')";
        $r2 = @mysqli_query($dbc, $q2);
        $r = @mysqli_query($dbc, $q); // Izvrsavanje upita.
        
        if ($r2) { // AKo je upit OK.
			...
			echo('Greska sa SQL serverom!');
			
			// Poruka za javnost:
			echo '<h1>Sistemska greška</h1>
			<p class="error">Nije uspela izmena destinacije!</p>';
